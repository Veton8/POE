class_name SeriousPunchAbility
extends Ability

# Saitama Q. Screen-piercing line nuke with massive damage and effective
# infinite pierce — represents the canonical "every enemy dies in one punch"
# joke. Long cooldown, screen shake, freeze-frame on cast.

@export var damage_mult: float = 30.0
@export var pierce: int = 99
@export var speed_mult: float = 2.5


func _ready() -> void:
	super._ready()
	ability_name = "Serious Punch"
...
func _activate() -> void:
	...
	Events.screen_shake.emit(20.0, 0.4)
